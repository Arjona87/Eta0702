...
const MUNICIPIO_COLORS = {
    'Guadalajara': '#2C3E50',
    'Zapopan': '#27AE60',
    'San Pedro Tlaquepaque': '#3498DB',
    'Tlajomulco de Zúñiga': '#F1C40F',
    'Tonala': '#E67E22',
    'El Salto': '#9B59B6',
    'Juanacatlán': '#95A5A6',
    'Ixtlahuacan de los Membrillos': '#E74C3C',
    'Zapotlanejo': '#34495E',
    'Interior del estado': '#000000'
};
...
function initializeApp() {
    // NO inicializar mapa aquí - se inicializará lazy al activar pestaña Carpetas
    // initializeMap();
    ...
    console.log('ℹ️ Mapa dinámico se inicializará al activar pestaña Carpetas');
}
...
function initializeMap() {
    ...
    // Inicializar capas
    markersLayer = L.layerGroup(); // NO agregar al mapa por defecto
...
function updateMapMarkers(data) {
    ...
    data.forEach(item => {
        ...
        // Determinar la forma del ícono según e        const color = YEAR_COLORS[item.año] || '#999999';
        
        // Símbolo + (cruz/plus) con color según año
        const iconShape = `<div style="color: ${color}; font-size: 20px; font-weight: bold; text-shadow: 0 0 2px white, 0 0 4px white; line-height: 1;">+</div>`;
            ...
            <div class="heatmap-container">
                <div class="checkbox-container">
                    <div class="checkbox-item">
                        ...
                        <label for="heatmapConcentracion">Homicidios Dolosos (C.I.)</label>
                    </div>
                    <div class="checkbox-item">
                        <input type="checkbox" id="heatmapVictimas">
                        <label for="heatmapVictimas">Mapa de calor</label>
                    </div>
                    <div class="checkbox-item">
...
function toggleHeatmapConcentracion() {
    ...
    if (checkbox.checked) {
        // Mostrar íconos de homicidios (+)
        if (markersLayer) {
            map.addLayer(markersLayer);
        }
        console.log('Homicidios Dolosos (C.I.) activados');
    } else {
        // Ocultar íconos de homicidios
        if (markersLayer) {
            map.removeLayer(markersLayer);
        }
        console.log('Homicidios Dolosos (C.I.) desactivados');
    }
}
...
function toggleHeatmapVictimas() {
    ...
    if (checkbox.checked) {
        ...
        heatmapVictimas = L.heatLayer(heatData, {
            radius: 25,  // Reducido de 35 a 25
            blur: 15,    // Aumentado de 8 a 15 para más difuminado
            maxZoom: 12,
            max: maxVictimas * 1.5,  // Aumentar max para reducir intensidad
            gradient: {
                0.0001: 'rgba(0, 0, 255, 0.02)',     // Azul más suave
                0.1: 'rgba(0, 255, 255, 0.1)',       // Cian más suave
                0.3: 'rgba(0, 255, 0, 0.15)',        // Verde más suave
                0.5: 'rgba(255, 255, 0, 0.2)',       // Amarillo más suave
                0.7: 'rgba(255, 165, 0, 0.3)',       // Naranja más suave
                1.0: 'rgba(255, 0, 0, 0.4)'          // Rojo más suave
            }
        }).addTo(map);
...
function setupYearSymbologyListeners() {
    ...
    // Event listener específico para checkbox TODOS (antes 2025p)
    const checkbox2025p = document.getElementById('show2025p');
    if (checkbox2025p) {
        checkbox2025p.addEventListener('change', function() {
            // Funcionalidad TODOS: marcar/desmarcar todos los años
...
function updateAnualChart(data) {
    console.log('📊 Iniciando updateAnualChart con', data.length, 'registros');
    ...
    const normalizarMunicipio = (nombre) => {
        return nombre
            .trim()
            .toLowerCase()
            .normalize('NFD')
    ...
    const municipiosAMGNormalizados = {};
    municipiosAMG.forEach(m => {
        municipiosAMGNormalizados[normalizarMunicipio(m)] = m;
    });
    
    console.log('📝 Municipios AMG normalizados:', municipiosAMGNormalizados);
    
    // Agrupar datos por año y municipio
    const anualData = {};
    const municipiosEncontrados = new Set();
    ...
    data.forEach(item => {
        ...
        // Normalizar el nombre del municipio del item
        const municipioNormalizado = normalizarMunicipio(item.municipio);
        municipiosEncontrados.add(item.municipio); // Guardar nombre original
        
        // Determinar si es municipio AMG o Interior del estado
        let municipioKey;
        if (municipiosAMGNormalizados[municipioNormalizado]) {
            municipioKey = municipiosAMGNormalizados[municipioNormalizado];
        } else {
            municipioKey = 'Interior del estado';
        }
        
        anualData[item.año][municipioKey] = (anualData[item.año][municipioKey] || 0) + item.victimas;
    });
    
    console.log('📋 Municipios encontrados en datos:', Array.from(municipiosEncontrados).slice(0, 20));
    console.log('📊 Datos agrupados por año:', anualData);
    ...
    // Crear datasets para cada municipio/grupo usando MUNICIPIO_COLORS
    const datasets = municipiosParaGrafico.map((municipio) => {
        const color = MUNICIPIO_COLORS[municipio] || '#999999';
        const dataArray = años.map(año => {
            return anualData[año] && anualData[año][municipio] ? anualData[año][municipio] : 0;
        });
        
        const total = dataArray.reduce((sum, val) => sum + val, 0);
        console.log(`🟢 ${municipio}: [${dataArray.join(', ')}] = ${total} total`);
    ...
    console.log('✅ Datasets creados:', datasets.length, 'municipios');
...
class RealTimeDataManager {
    constructor() {
        this.googleSheetsUrl = 'https://docs.google.com/spreadsheets/d/1I6eGs9L4vICNaIAxr2yJi7953ch6fUyIRV-G-EHdI78/export?format=csv&gid=594361153';
        this.updateInterval = 2500; // 2.5 segundos (optimizado +50%)
    ...
    createStatusIndicator() {
        ...
        if (headerContent) {
            ...
            document.getElementById('manual-update').addEventListener('click', () => {
                ...
                // Actualizar mapa dinámico también
                setTimeout(() => {
                    if (typeof map !== 'undefined') {
                        ...
                        console.log('✓ Mapa dinámico actualizado manualmente');
                    }
                }, 200);
            });
        }
